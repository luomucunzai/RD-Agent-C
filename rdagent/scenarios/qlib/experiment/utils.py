...
def generate_data_folder_from_crypto():
    """Generate crypto H5 data from Binance CSV (no $factor column)."""
    template_path = Path(__file__).parent / "factor_data_template"
    import subprocess
    result = subprocess.run(
        [sys.executable, str(template_path / "generate_crypto.py")],
        capture_output=True, text=True,
    )
    if result.returncode != 0:
        # Try debug version if full fails
        print(f"generate_crypto.py (full) failed: {result.stderr}")
        result = subprocess.run(
            [sys.executable, str(template_path / "generate_crypto.py"), "--debug"],
            capture_output=True, text=True,
        )
        assert result.returncode == 0, (
            f"generate_crypto.py failed: {result.stderr}\n"
            "Please run downdata.py first to download Binance crypto data."
        )

    assert (template_path / "daily_pv_all.h5").exists() or (template_path / "daily_pv_debug.h5").exists(), (
        "Crypto H5 data not generated. Run generate_crypto.py manually."
    )

    # Copy to data folders
    for src_name, dst_folder in [("daily_pv_all.h5", FACTOR_COSTEER_SETTINGS.data_folder),
                                  ("daily_pv_debug.h5", FACTOR_COSTEER_SETTINGS.data_folder_debug)]:
        src = template_path / src_name
        if src.exists():
            Path(dst_folder).mkdir(parents=True, exist_ok=True)
            shutil.copy(src, Path(dst_folder) / "daily_pv.h5")

    # Copy README
    for dst_folder in [FACTOR_COSTEER_SETTINGS.data_folder, FACTOR_COSTEER_SETTINGS.data_folder_debug]:
        Path(dst_folder).mkdir(parents=True, exist_ok=True)
        shutil.copy(template_path / "README.md", Path(dst_folder) / "README.md")
...
def get_data_folder_intro(fname_reg: str = ".*", flags=0, variable_mapping=None) -> str:
    ...
    if (
        ...
        if os.environ.get("CRYPTO_MODE", "").lower() == "true":
            generate_data_folder_from_crypto()
        else:
    ...
    # CRYPTO_MODE: appends context about continuous 7x24 trading and no adjustment factor
    if os.environ.get("CRYPTO_MODE", "").lower() == "true":
        intro += (
            "\n\n=== CRYPTO_MODE ACTIVE ===\n"
            "- This is cryptocurrency market data: 7x24 continuous trading, no weekend or holiday gaps.\n"
            "- There is NO 'factor' (adjustment factor) column — no ex-rights, no dividend adjustments needed.\n"
            "- Timestamps are continuous and sequential across all days.\n"
            "- The 'instrument' in the index represents trading pair symbols (e.g. BTCUSDT, ETHUSDT), not stock codes.\n"
        )
