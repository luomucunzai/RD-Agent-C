...
class LiteLLMAPIBackend(APIBackend):
    ...
    def _create_embedding_inner_function(self, input_content_list: list[str]) -> list[list[float]]:
        ...
        For litellm_proxy/ prefixed models, use OpenAI client directly
        (LiteLLM's proxy handler sends invalid params like encoding_format=None).
        """
        ...
        # litellm_proxy/ prefix -> use OpenAI client directly (avoids LiteLLM proxy parameter issues)
        if model_name.startswith("litellm_proxy/"):
            actual_model = model_name[len("litellm_proxy/"):]
            proxy_api_key = os.environ.get("LITELLM_PROXY_API_KEY", "")
            proxy_api_base = os.environ.get("LITELLM_PROXY_API_BASE", "")
            if proxy_api_key and proxy_api_base:
                from openai import OpenAI
                client = OpenAI(api_key=proxy_api_key, base_url=proxy_api_base)
                response = client.embeddings.create(model=actual_model, input=input_content_list)
